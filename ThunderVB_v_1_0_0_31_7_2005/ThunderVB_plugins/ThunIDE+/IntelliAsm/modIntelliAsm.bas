...
Global asmTT As New frmTip
Global bDoNotHideOfFL As Boolean
Global wm_tthide As Boolean
...
Function GetCurContext() As String
   ...
   GPF_Set GPF_RaiseErr, "modIntelliAsm", "GetCurContext", PLUGIN_NAME
       ...
       If scol > 0 Then
          ...
          Else
           '    curline = curline & space$(scol - Len(curline))
          End If
      Else
          curline = ""
      End If
      ...
      wordC = ArrUBound(wa) + 1 'Word count
          
              
      If wordC < 2 Then 'nah.. not much type so propably still on the first word
          GetCurContext = "asml1" 'default to this case the user is still on the first word
      Else
...
Exit_Function:
  ...
  Exit Function
ErrH:
      ErrorBox Err.Description & " at line " & Erl, "modIntelliAsm", "GetCurContext"
  GPF_Reset
End Function
...
Function GetListFromContext(con As String, style As Isml_kw_types) As LTI
   ...
   'Error handling
   On Error GoTo ErrH
   GPF_Set GPF_RaiseErr, "modIntelliAsm", "GetListFromContext", PLUGIN_NAME
       ...
       Select Case con
          ...
          Case Else 'we have a numeric list with the possible tooltips/lists
              ...
              ReDim tl(ArrUBound(tls)) 'and proccess it
              tl(0) = tls(0)
              For i = 1 To ArrUBound(tls)
                  tl(i) = tls(i)
  ...
  Exit Function
ErrH:
  ErrorBox Err.Description & " at line " & Erl, "modIntelliAsm", "GetListFromContext"
  GPF_Reset
End Function
...
Public Function GetWordFormPos(ByVal slin As Long, ByVal scol As Long, Optional ByRef ws As Long, Optional ByRef we As Long) As String
   On Error GoTo ErrH
   GPF_Set GPF_RaiseErr, "modIntelliAsm", "GetCurCarWord", PLUGIN_NAME
...
Exit_Function:
  ...
  Exit Function
ErrH:
  ErrorBox Err.Description & " at line " & Erl, "modIntelliAsm", "GetWordFormPos"
  GPF_Reset
End Function
      ...
      If style And kw_Tipform Then
          If bMakenewTT Then
              ...
              If Len(temp.ttInfo) Then
                  ...
                  t2 = ""
                  For i = temp.curLVL To ArrUBound(t)
                      t2 = t2 & t(i) & " "
                  Next i
                  asmTT.pbc.Print t2;
              End If
...
Public Sub AutoExpandAsm_C()
   ...
   GPF_Set GPF_RaiseErr, "modIntelliAsm", "AutoExpandAsm", PLUGIN_NAME
