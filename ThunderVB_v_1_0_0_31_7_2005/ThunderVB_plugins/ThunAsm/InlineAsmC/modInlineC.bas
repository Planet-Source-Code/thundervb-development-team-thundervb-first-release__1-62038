...
Public Type StringCol
    items() As String
    count As Long
End Type

Global libs As StringCol
    ...
    For i = 0 To UBound(asm)
        If Len(asm(i)) > 0 Then
            ...
            If InStr(1, temp, "'#c'", vbTextCompare) = 1 Then
                ...
                temp2.fnameC = GetCNameFromVB(temp2.fName) ' GetFirstWord(Replace(temp, "(", " "))
...
Public Function GetCNameFromVB(ByVal vbname As String) As String

    LogMsg "VB Name=" & vbname, "modInlineC", "GetCNameFromVB"
    vbname = Mid$(vbname, 2)
    GetCNameFromVB = Left$(vbname, InStr(1, vbname, "@", vbBinaryCompare) - 1)
    LogMsg "C Name=" & GetCNameFromVB, "modInlineC", "GetCNameFromVB"
...
Dim c_code As String, s_out As String, temp As String, cl_exe As String, fl_ As String
    ...
    If Len(cl_exe) > 0 And FileExist(cl_exe) Then
         ...
         '/I" & Add34(Get_Paths(INCFiles_Directory))
        If ExecuteCommand(Add34(cl_exe) & " " & Add34(file_c) & " /MD /G6  /Ox  /Gz /GA /FAs /Fa" & Add34(file_asm_out) & " /c", s_out, GetPath(cl_exe)) = False Then
...
Dim fl As Boolean
    ...
    For i = 0 To UBound(lines)
        temp = Trim(lines(i))
        'proccess it
        If InStr(1, temp, "'#c'lib=", vbTextCompare) = 1 Then ' c external lib directive
            AddLib Right(temp, Len(temp) - Len("'#c'lib="))
        ElseIf InStr(1, temp, "'#c'", vbTextCompare) = 1 Then ' c code line
            If Get_General(GenerateAsmCHeaders) Then
                If fl Then
                    AppendString out, "#include " & Add34(ch_path) & vbNewLine
                    fl = False
                End If
            End If
...
Dim i As Long, temp As String, tmp2() As String
    ...
    i = InStr(1, asm, "ASSUME", vbTextCompare)
    temp = Mid$(asm, i, Len(asm) - i)
    tmp2 = Split(temp, vbNewLine)
    
    If tmp2(1) <> "endif" Then
        WarnBox "Error locating C listing end ...", "modInlineC", "GetAsmCodeFromCListing"
    Else
        tmp2(0) = ""
        tmp2(1) = ""
...
Private Function GetRemAsmCode(str() As String, ByVal i As Long) As String
Dim curseg As String, i2 As Long, out As String_B, seg_s As Long
    ...
    If InStr(1, str(seg_s - 2), "CONST" & vbTab & "ENDS") Then
        For i2 = seg_s - 1 To 0 Step -1
            If Len(str(i2)) Then
                If InStr(1, str(i2), "CONST" & vbTab & "SEGMENT") Then
                    str(i2) = ""
                    Exit For
                End If
                str(i2) = ""
...
Public Function AddCLibs(ByRef CmdLine As String)
    
    'not needed
    'CmdLine = Replace(CmdLine, "OPT:REF", "OPT:NOREF")
    
    Dim i As Long
    
    For i = 0 To libs.count - 1
        CmdLine = CmdLine & "  " & libs.items(i)
    Next i
...
Public Sub AddLib(nam As String)
Dim i As Long

    For i = 0 To libs.count - 1
        If LCase$(libs.items(i)) = nam Then
            Exit Sub
        End If
    Next i
    
    ReDim Preserve libs.items(libs.count)
    libs.items(libs.count) = nam
    libs.count = libs.count + 1
    
End Sub

'Old way [to CRT dll ;)]
'Kept here just to see how the ideas are implemented/developed
'
'
...
' second word of the first c line
'; 94   : '#c'int Killed(int i,int i2){
